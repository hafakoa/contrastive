...
def DataTransform_TD(sample):
    ...
    aug_1 = jitter(sample, sigma=0.8)                 #augmentation.jitter_ratio
    aug_2 = scaling(sample, sigma=1.1)
    aug = aug_1 + aug_2
    return aug

def DataTransform_TD_bank(sample):
    ...
    aug_1 = jitter(sample, sigma=0.8)               #augmentation.jitter_ratio
    aug_2 = scaling(sample, sigma=1.1)              #augmentation.jitter_scale_ratio
    aug_3 = permutation(sample, max_segments=5)     #augmentation.max_seg
...
def remove_frequency(x, pertub_ratio=0.0):
    ...
    #mask = torch.empty(x.shape, dtype=torch.float32, device='cuda').uniform_() > pertub_ratio
    mask = torch.rand(x.shape, dtype=torch.float32, device=x.device) > pertub_ratio
...
def add_frequency(x, pertub_ratio=0.0):
    ...
    mask = torch.rand(x.shape, dtype=torch.float32, device=x.device) > (1-pertub_ratio)
    mask = mask.to(x.device)
    max_amplitude = x.max()
...
def scaling(x, sigma=1.1):
    ...
    for i in range(x.shape[1]):
        xi = x[:, i, :]
        scaled = (xi * factor).unsqueeze(1)  # Add sequence dimension back
        ai.append(scaled)
    return torch.cat(ai, dim=1).numpy()
